...
def is_enough_free_locations(curr_node, goal_node, nodes_dict: dict, h_dict: Dict[str, np.ndarray], other_curr_nodes: list, non_sv_nodes_np: np.ndarray) -> Tuple[bool, str]:
    ...
    next_node = min_nei_node
    sv_count = 0
    sv_list = []
    while next_node != goal_node:
        if not non_sv_nodes_np[next_node.x, next_node.y]:
            sv_count += 1
        else:
    ...
    while len(open_list) > 0:
        ...
        if non_sv_nodes_np[next_node.x, next_node.y] and next_node not in other_curr_nodes:
            free_count += 1
            if free_count >= max_corridor:
                return True, 'good'
        ...
        for nei_name in next_node.neighbours:
            ...
            if nei_node in open_list:
                continue
            heapq.heappush(open_list, nei_node)
        heapq.heappush(closed_list, next_node)
